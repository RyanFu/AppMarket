...
public abstract class PublicActivity extends Activity {
	 ...
	 * @param info
	 */
	public abstract void onAppInstallOrUninstallDone(int status,
			PackageInfo info);
	 ...
	 * @param appId
	 */
	public abstract void onAppStatusChange(boolean isCancel,
	...
	private void registerAllReceiver() {
		...
		intentFilter
				.addAction(DownloadConstDefine.BROADCAST_ACTION_ADD_DOWNLOAD);
		intentFilter
				.addAction(DownloadConstDefine.BROADCAST_ACTION_CANCEL_DOWNLOAD);
		intentFilter
				.addAction(DownloadConstDefine.BROADCAST_ACTION_COMPLETE_DOWNLOAD);
		intentFilter
				.addAction(DownloadConstDefine.BROADCAST_ACTION_UPDATE_DATA_MERGE_DONE);
		intentFilter.addAction(AConstDefine.SAVE_FLOW_BROADCAST);
	...
	protected void onResume() {
		...
		System.out
				.println("===========isEconomizeTraffic" + isEconomizeTraffic);
	}
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (DownloadService.mDownloadService != null) {
					...
					for (int j = 0; j < downloadList.size(); j++) {
						...
						if (item.packageName.equals(entity.packageName)
								&& item.versionCode == entity.versionCode) {
				...
				if (infos != null && infos.size() > 0) {
					for (int k = 0; k < infos.size(); k++) {
						...
						if (info.packageName.equals(items.get(i).packageName)
								&& info.versionCode >= items.get(i).versionCode) {
										...
										if (historyItem.versionCode == entity.versionCode) {
											if (info != null) {
												if (info.versionCode < historyItem.versionCode) {
													if (entity.getStatus() != DownloadConstDefine.STATUS_OF_INITIAL
															&& entity
																	.getStatus() != DownloadConstDefine.STATUS_OF_IGNORE) {
									...
									case DownloadConstDefine.TYPE_OF_UPDATE:
										if (info != null) {
											if (info.versionCode < historyItem.versionCode) {
												...
												System.out
														.println("============= TYPE_OF_UPDATE, "
																+ historyItem.appName
																+ ", "
																+ historyItem.versionName
																+ ", "
																+ historyItem.versionCode
																+ ", info_versionCode"
																+ info.versionCode
																+ ", "
																+ historyItem.url);
			...
			if (infos != null && infos.size() > 0) {
				for (int k = 0; k < infos.size(); k++) {
					...
					if (info.packageName.equals(item.packageName)
							&& info.versionCode >= item.versionCode) {
	...
	private class ApkStatusReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			 ...
			 * true)); }
			 */
			if (DownloadConstDefine.BROADCAST_ACTION_ADD_DOWNLOAD.equals(intent
					.getAction())) {
				Bundle bundle = intent.getExtras();
				if (bundle != null) {
					DownloadEntity entity = bundle
							.getParcelable(DownloadConstDefine.DOWNLOAD_ENTITY);
					if (entity != null) {
						onAppStatusChange(false, entity.packageName,
								entity.versionCode);
					}
				}
			} else if (DownloadConstDefine.BROADCAST_ACTION_CANCEL_DOWNLOAD
				...
				if (bundle != null) {
					DownloadEntity entity = bundle
							.getParcelable(DownloadConstDefine.DOWNLOAD_ENTITY);
					if (entity != null) {
						onAppStatusChange(true, entity.packageName,
								entity.versionCode);
					}
				}
			} else if (DownloadConstDefine.BROADCAST_ACTION_COMPLETE_DOWNLOAD
					.equals(intent.getAction())) {
				Bundle bundle = intent.getExtras();
				if (bundle != null) {
					DownloadEntity entity = bundle
			...
			} else if (DownloadConstDefine.BROADCAST_ACTION_UPDATE_DATA_MERGE_DONE
					.equals(intent.getAction())) {
				onUpdateDataDone();
			} else if (AConstDefine.SAVE_FLOW_BROADCAST.equals(intent
					.getAction())) {
				isRemoteImage = !(intent.getBooleanExtra("save_flow_status",
						true));
				...
				if (wifiNetworkInfo.isAvailable()
					...
					System.out.println("==========wifi isEconomizeTraffic:"
							+ isEconomizeTraffic + ", isRemoteImage:"
							+ isRemoteImage);
					if (isEconomizeTraffic) {
						isRemoteImage = true;
						loadingImage();
					}
				} else if (mobileNetworkInfo.isAvailable()
						&& mobileNetworkInfo.isConnected()) {
					System.out.println("==========mobile isEconomizeTraffic:"
							+ isEconomizeTraffic + ", isRemoteImage:"
							+ isRemoteImage);
					if (isEconomizeTraffic) {
						isRemoteImage = false;
	...
	private BroadcastReceiver mPackageStatusReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(AConstDefine.BROADCAST_SYS_ACTION_APPINSTALL)
					|| action
							.equals(AConstDefine.BROADCAST_SYS_ACTION_APPREMOVE)) {
				String packageName = intent.getDataString();
				if (!TextUtils.isEmpty(packageName)) {
					packageName = packageName.substring(packageName
							.indexOf(PACKAGE_STR) + PACKAGE_STR.length());
				}
				...
				if (action.equals(AConstDefine.BROADCAST_SYS_ACTION_APPINSTALL)) {
					...
					List<ADownloadApkItem> updatingList = ADownloadService.updateAPKList.apkList;
					if (updatingList != null) {
						for (int i = 0; i < updatingList.size(); i++) {
							ADownloadApkItem updateItem = updatingList.get(i);
							if (updateItem.apkPackageName.equals(packageName)) {
					...
					onAppInstallOrUninstallDone(UNINSTALL_APP_DONE, info);
				}
			}
		}
	};
