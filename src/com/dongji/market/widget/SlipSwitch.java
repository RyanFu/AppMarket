...
public class SlipSwitch extends View implements OnTouchListener {
	...
	// 从xml文件中获取的各种参数
	private Drawable switch_on_background, switch_off_background, switch_background;
	...
	public SlipSwitch(Context context, AttributeSet attrs) {
		...
		TypedArray typeArray = context.obtainStyledAttributes(attrs, R.styleable.SlipSwitch);
		switch_background = typeArray.getDrawable(R.styleable.SlipSwitch_background);
		switch_on_background = typeArray.getDrawable(R.styleable.SlipSwitch_onBackground);
		switch_off_background = typeArray.getDrawable(R.styleable.SlipSwitch_offBackground);

		// System.out.println("width===" + switchWidth + "     height===" +
		// switchHeight);
		// Toast.makeText(context, "width===" + switchWidth + "     height===" +
		// switchHeight, Toast.LENGTH_SHORT).show();
		onText = typeArray.getString(R.styleable.SlipSwitch_onText);
		offText = typeArray.getString(R.styleable.SlipSwitch_offText);
		textSize = typeArray.getDimensionPixelSize(R.styleable.SlipSwitch_textSize, 15);
		onTextColor = typeArray.getColor(R.styleable.SlipSwitch_onTextColor, 0xfff);
		offTextColor = typeArray.getColor(R.styleable.SlipSwitch_offTextColor, 0x000);
		switchState = typeArray.getBoolean(R.styleable.SlipSwitch_switchState, false);
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		setImageResource(getWidth(), getHeight(), drawableToBitmap(switch_background), drawableToBitmap(switch_on_background), drawableToBitmap(switch_off_background), onText, offText, textSize, onTextColor, offTextColor);
	}
	...
	public void setImageResource(float switchWidth, float switchHeight, Bitmap switchBg, Bitmap switchOnBg, Bitmap switchOffBg, String onText, String offText, int textSize, int onTextColor, int offTextColor) {
		switch_off_bg = drawTextAtBitmap(changeBitmapSize(switchOffBg, switchWidth / 2, switchHeight), offText, offTextColor, textSize);
		switch_on_bg = drawTextAtBitmap(changeBitmapSize(switchOnBg, switchWidth / 2, switchHeight), onText, onTextColor, textSize);
		switch_bg = changeBitmapSize(switchBg, switchWidth, switchHeight);

		// 右半边的 Rect,即滑动按钮在右半边时表示开关开启
		on_rect = new Rect(switch_bg.getWidth() - switch_on_bg.getWidth(), 0, switch_bg.getWidth(), switch_on_bg.getHeight());
		// 左半边的Rect,即滑动按钮在左半边时表示开关关闭
		off_rect = new Rect(0, 0, switch_off_bg.getWidth(), switch_off_bg.getHeight());
		invalidate();
	}

	/*
	 * public void setImageResource(int switchBg, int switchOnBg, int
	 * switchOffBg) { switch_off_bg = drawTextAtBitmap(
	 * changeBitmapSize(BitmapFactory.decodeResource(getResources(),
	 * switchOffBg), 0.5f), closeText, Color.BLACK, 20); switch_on_bg =
	 * drawTextAtBitmap(
	 * changeBitmapSize(BitmapFactory.decodeResource(getResources(),
	 * switchOnBg), 0.5f), openText, Color.WHITE, 20); switch_bg =
	 * changeBitmapSize( BitmapFactory.decodeResource(getResources(), switchBg),
	 * 0.5f);
	 * 
	 * // 右半边的 Rect,即滑动按钮在右半边时表示开关开启 on_rect = new Rect(switch_bg.getWidth() -
	 * switch_on_bg.getWidth(), 0, switch_bg.getWidth(),
	 * switch_on_bg.getHeight()); // 左半边的Rect,即滑动按钮在左半边时表示开关关闭 off_rect = new
	 * Rect(0, 0, switch_off_bg.getWidth(), switch_off_bg.getHeight());
	 * invalidate(); }
	 */
	...
	public void onDraw(Canvas canvas) {
		...
		if (left_slipBtn < 0) {
			left_slipBtn = 0;
		} else if (left_slipBtn > switch_bg.getWidth() - switch_on_bg.getWidth()) {
			left_slipBtn = switch_bg.getWidth() - switch_on_bg.getWidth();
		}
	 ...
	 * @return
	 */
	private Bitmap drawTextAtBitmap(Bitmap bitmap, String text, int textColor, int textSize) {
		...
		canvas.drawText(text, (width - textWidth) / 2, (height + textHeight) / 2 - textSize / 4, paint);
	...
	private Bitmap changeBitmapSize(Bitmap bitmap, float width, float height) {
		...
		/*
		 * Canvas canvas = new Canvas(newbit); Paint paint = new Paint();
		 * canvas.drawBitmap(bitmap, 0, 0, paint);
		 * 
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:// 按下
			if (event.getX() > switch_bg.getWidth() || event.getY() > switch_bg.getHeight()) {
				return false;
			}
			isSlipping = true;
