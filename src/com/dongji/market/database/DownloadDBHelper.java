...
public class DownloadDBHelper {
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("create table if not exists " + DOWNLOAD_TABLE
					+ "(_id INTEGER primary key autoincrement, "
					+ DownloadDBConstDefine.DOWNLOAD_APPNAME + " TEXT, "
					+ DownloadDBConstDefine.DOWNLOAD_APPID + " INTEGER, " + ""
					+ DownloadDBConstDefine.DOWNLOAD_CATEGOTY + " INTEGER, "
					+ DownloadDBConstDefine.DOWNLOAD_CURRENT_POSITION
					+ " LONG, " + DownloadDBConstDefine.DOWNLOAD_FILE_LENGTH
					+ " LONG, " + ""
					+ DownloadDBConstDefine.DOWNLOAD_PACKAGENAME + " TEXT, "
					+ DownloadDBConstDefine.DOWNLOAD_VERSION_CODE
					+ " Integer, " + DownloadDBConstDefine.DOWNLOAD_APKURL
					+ " TEXT, " + DownloadDBConstDefine.DOWNLOAD_ICON_URL
					+ " TEXT, " + DownloadDBConstDefine.DOWNLOAD_VERSION_NAME
					+ " TEXT, " + "" + DownloadDBConstDefine.DOWNLOAD_TYPE
					+ " Integer, " + DownloadDBConstDefine.DOWNLOAD_STATUS
					+ " Integer, " + DownloadDBConstDefine.HEAVY
					+ " Integer);");
//			db.execSQL()
		}
	...
	public void addOrUpdateDownload(DownloadEntity entity) {
		...
		try{
			...
			cursor = sqLiteDatabase
					.query(DOWNLOAD_TABLE, null,
							DownloadDBConstDefine.DOWNLOAD_APPID + "=? AND "
									+ DownloadDBConstDefine.DOWNLOAD_CATEGOTY
									+ "=?", new String[] {
									String.valueOf(entity.appId),
									String.valueOf(entity.category) }, null,
							null, null);
			if(cursor.getCount()>0) {
				sqLiteDatabase.update(DOWNLOAD_TABLE, values,
						DownloadDBConstDefine.DOWNLOAD_APPID + "=? AND "
								+ DownloadDBConstDefine.DOWNLOAD_CATEGOTY
								+ "=?", new String[] {
								String.valueOf(entity.appId),
								String.valueOf(entity.category) });
			}else {
	...
	public boolean deleteDownloadEntity(DownloadEntity entity) {
		...
		try{
			...
			int result=sqLiteDatabase.delete(DOWNLOAD_TABLE,
					DownloadDBConstDefine.DOWNLOAD_APPID + "=? AND "
							+ DownloadDBConstDefine.DOWNLOAD_CATEGOTY + "=?",
					new String[] { String.valueOf(entity.appId),
							String.valueOf(entity.category) });
			return result > 0;
		}catch(SQLiteException e) {
