...
//	private 
	...
	public void onStart(Intent intent, int startId) {
		...
		/*boolean isUpdateTimeOut=isUpdateTimeOut();
		if (isUpdateTimeOut) {
		...
		if(updateList!=null && updateList.size()>0) {
			...
			for(int i=0;i<count;i++) {
				if ((infos.get(i).applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) == 0
						&& (infos.get(i).applicationInfo.flags & ApplicationInfo.FLAG_UPDATED_SYSTEM_APP) == 0) {
					k++;
				}
			}
			int num=mApp.getUpdateList().size();
			int n=num-k;
			if(n>0) {
...
//			Intent intent=new Intent(AConstDefine.BROADCAST_REQUEST_UPDATE_ACTION);
...
//					requestUpdateData();
				} else if (isMobileUpdate && mobileNetworkInfo.isAvailable()
...
//					requestUpdateData();
	...
	private void onNetworkStatusReceiver(Context context) {
		if (mApp.getUpdateList() == null
				&& currentRetry < MAX_REQUEST_UPDATE_RETRY_NUM && isNetwork) {
			ConnectivityManager connectivityManager = (ConnectivityManager) context
					.getSystemService(Context.CONNECTIVITY_SERVICE);
	...
	private void initLastUpdateTime() {
		if(lastUpdateTime==0) {
			SharedPreferences mSharedPreferences = getSharedPreferences(
					ShareParams.SHARE_FILE_NAME, Context.MODE_PRIVATE);
