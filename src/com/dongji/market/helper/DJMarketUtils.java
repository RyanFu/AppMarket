...
//	public static boolean IS_INSTALLING = false;
	...
	public static List<InstalledAppInfo> getInstalledAppsByFlag(
		...
		if (flag == SoftwareMove_list_Activity.FLAG_PHONECARD) {
			for (PackageInfo pInfo : packages) {
				...
				if ((info.flags & ApplicationInfo.FLAG_SYSTEM) == 0
						...
						&& info.sourceDir.substring(0, 2).equals("/d")) {
			...
			for (PackageInfo pInfo : packages) {
				...
				if ((info.flags & ApplicationInfo.FLAG_SYSTEM) == 0
						&& (info.flags & ApplicationInfo.FLAG_UPDATED_SYSTEM_APP) == 0
						&& !info.packageName.equals("com.dongji.market")
						&& info.sourceDir.substring(0, 2).equals("/m")) {
					installedAppInfo.setAppInfo(info);
					// installedAppInfo.setIcon(info.loadIcon(pm));
					installedAppInfo.setName(info.loadLabel(pm) + "");
					installedAppInfo.setVersion(pInfo.versionName);
...
//						+ info.loadLabel(pm));
	...
	public static void checkDownload(Context context, ApkItem apkItem,
		...
		if (NetTool.checkIsDownload(context, apkItem.packageName + "_"
				+ apkItem.versionCode)) {
			NetTool.installApp(context, apkItem.packageName + "_"
					+ apkItem.versionCode);
			return;
		}
		ADownloadApkDBHelper aDownloadApkDBHelper = new ADownloadApkDBHelper(
				context);
		if (aDownloadApkDBHelper.selectApkIsExist(apkItem.packageName,
				apkItem.versionCode)
				&& AndroidUtils.checkFileExists(NetTool.DOWNLOADPATH
						+ apkItem.appId)) {
		...
		case DJMarketUtils.STATUS_ONLY_MOBILE:
			...
			if (isPromptUser) {
				...
				showMobileDownloadDialog(context, apkItem, mTextView, listener,
						map);
			}
			break;
	...
	public static void cancelListDownload(Context context, ApkItem item) {
		...
		if (item.status == STATUS_APK_INSTALL) {
			...
			intent.putExtra(BROADCAST_CANCELDOWNLOAD, item.packageName + "_"
					+ item.versionCode);
		} else if (item.status == STATUS_APK_UPDATE) {
			intent = new Intent(BROADCAST_ACTION_UPDATE);
			intent.putExtra(BROADCAST_CANCELUPDATE, item.packageName + "_"
					+ item.versionCode);
		}
		context.sendBroadcast(intent);
		db.deleteDownloadByPAndV(item.packageName, item.versionCode);
		NetTool.deleteFileByApkSaveName(item.packageName + "_"
				+ item.versionCode);
	}

	private static void prepareDownload(Context context, ApkItem apkItem,
			TextView mTextView, OnDownloadChangeStatusListener listener,
			Map<String, Object> map) {
		int apkType = getApkType(apkItem.packageName, apkItem.versionCode);
		if (apkType == STATUS_APK_UNINSTALL) {
			NetTool.startServiceToDownload(context, new ADownloadApkItem(
					apkItem, STATUS_OF_PREPAREDOWNLOAD));
		} else if (apkType == STATUS_APK_UNUPDATE) {
			NetTool.startServiceToDownload(context, new ADownloadApkItem(
					apkItem, STATUS_OF_PREPAREUPDATE));
		}
		if (listener != null) {
		...
		if (null != ADownloadService.updateAPKList) {
			for (int i = 0; i < ADownloadService.updateAPKList.apkList.size(); i++) {
				if (apkPackageName
						...
						&& apkVersionCode == ADownloadService.updateAPKList.apkList
								.get(i).apkVersionCode) {
	...
	public static void checkDownload(Context context, int downloadType) {
		...
		case DJMarketUtils.STATUS_ONLY_MOBILE:
			...
			if (downloadType == FLAG_ONEKEYUPDATEING) {
				isPromptUser = ((ADownloadActivity) context)
						.is3GDownloadPromptUser();
			} else {
				isPromptUser = ((MainActivity) context)
						.is3GDownloadPromptUser();
			}
			if (isPromptUser) {
				if (ADownloadService.canUse3GDownload()) {
	...
	public static void checkDownload(Context context,
		...
		if (NetTool.checkIsDownload(context, aDownloadApkItem.apkPackageName
			...
			NetTool.installApp(context, aDownloadApkItem.apkPackageName + "_"
	...
	private static void prepareDownload(Context context,
			ADownloadApkItem aDownloadApkItem) {
		System.out.println("状态——————" + aDownloadApkItem.apkStatus);
		if (aDownloadApkItem.apkStatus == STATUS_OF_PAUSE_BYHAND
			...
			intent.putExtra(BROADCAST_CONTINUEUPDATE,
					aDownloadApkItem.apkPackageName + "_"
							+ aDownloadApkItem.apkVersionCode);
			context.sendBroadcast(intent);
	...
	private static void showMobileDownloadDialog(final Context context,
			final ApkItem apkItem, final TextView mTextView,
			final OnDownloadChangeStatusListener listener,
		...
		if (!((Activity) context).isFinishing()) {
			...
			mDialog.setNeutralButton(
					context.getString(R.string.prompt_download),
					new View.OnClickListener() {
						@Override
						public void onClick(View v) {
							if (ADownloadService.canUse3GDownload()) { // 判断当前用户使用3G下载有否流量限制
								prepareDownload(context, apkItem, mTextView,
										listener, map);
								...
								try {
									...
									((BaseActivity) context)
											.set3GDownloadPromptUser();
									flag = 1;
								}
								if (flag == 0) {
									((ApkDetailActivity) context)
											.set3GDownloadPromptUser();
								}
							} else {
								// showFlowSettingDialog(context);
								Intent intent = new Intent(
										BROADCAST_ACTION_NOFLOW);
								context.sendBroadcast(intent);
							}
							mDialog.dismiss();
						}
	...
	private static void showMobileDownloadDialog(final Context context,
			final ADownloadApkItem aDownloadApkItem) {
		if (!((Activity) context).isFinishing()) {
			final CustomNoTitleDialog mDialog = new CustomNoTitleDialog(context);
	...
	private static void showMobileDownloadDialog(final Context context,
			final int downloadType, final ArrayList<ApkItem> apkItems) {
		if (!((Activity) context).isFinishing()) {
			final CustomNoTitleDialog mDialog = new CustomNoTitleDialog(context);
			...
			mDialog.setNeutralButton(
					...
					new View.OnClickListener() {
						...
						public void onClick(View v) {
							if (ADownloadService.canUse3GDownload()) { // 判断当前用户使用3G下载有否流量限制
								prepareDownload(context, downloadType, apkItems);
								((SoftwareManageActivity) context)
										.set3GDownloadPromptUser();
	...
	public static boolean isExceedDate(Context context) {
		SharedPreferences mSharedPreferences = context.getSharedPreferences(
				context.getPackageName() + "_temp", Context.MODE_PRIVATE);
	...
	public static long queryUse3GDownloadSize(Context context) {
		/*if (ADownloadService.isSelfStart()) {
			return ADownloadService.get3GDownloadSize();
		} else {
			SharedPreferences pref = context.getSharedPreferences(
					AConstDefine.DONGJI_SHAREPREFERENCES, Context.MODE_PRIVATE);
			long size = pref.getLong(AConstDefine.SHARE_DOWNLOADSIZE, 0);
			return size;
		}*/
	...
	public static String convertionInstallNumber(Context context, long num) {
		...
		if (num < 10000) {
			...
			return value.substring(0, value.length() - 4)
					+ context.getString(R.string.myriad);
		} else {
	...
	public static boolean isLogin(Context context) {
		LoginParams loginParams = ((AppMarket) context.getApplicationContext())
				.getLoginParams();
	...
	public static String convertPackageName(String packageName) {
		...
		if (num != -1) {
			return packageName.substring(PACKAGE_STR.length() + num,
					packageName.length());
		}
		return packageName;
	}
	 ...
	 * @return
	 */
	public static InstalledAppInfo getInstalledAppInfoByPackageName(
	 ...
	 * @return
	 */
	public static int getInstalledAppVersionCodeByPackageName(Context context,
			String packageName) {
		PackageManager pm = context.getPackageManager();
		List<PackageInfo> packages = pm.getInstalledPackages(0);
		for (PackageInfo pInfo : packages) {
	...
	public static List<InstalledAppInfo> getBackupItemList(Context context) {
		...
		if (directory.exists()) {
			...
			for (i = 0; i < files.length; i++) {
				...
				InstalledAppInfo tempBackupItemInfo = getApkFileInfo(context,
						NetTool.BACKUPPATH + files[i].getName());
	 ...
	 * @return
	 */
	public static InstalledAppInfo getApkFileInfo(Context context,
			String apkPath) {
		...
		try {
			...
			if (info != null) {
				...
				if (info.labelRes != 0) {
					...
					backupItemInfo.setName(apkName.substring(0,
							apkName.lastIndexOf(".")));
				}
			} else {
				return null;
	...
	public static void sinaLogin(Activity context, Handler handler) {
		...
		if (AndroidUtils.isNetworkAvailable(context)) {
			...
			// 应用回调页不可为空
			weibo.authorize(context, new SinaOAuthDialogListener(context,
					handler, CONSUMER_KEY, CONSUMER_SECRET));
		} else {
	...
	public static void getTencentUsrInfo(Context context, OAuthV2 oAuth,
			Handler handler) {
